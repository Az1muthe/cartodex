...
:root{
  ...
  --red:#c82828;--red2:#e03a3a;--orange:#c86030;--orange2:#e07840;
  --gold:#c8a448;--gold2:#e8c060;--green:#3ab870;
...
.logo{font-family:'Fredoka',sans-serif;font-weight:700;font-size:1.55rem;letter-spacing:3px;text-transform:uppercase;
  color:#f5ece0;-webkit-text-stroke:1.5px var(--red);
  text-shadow:0 0 24px rgba(200,40,40,.4),2px 2px 0 rgba(60,5,5,.9);
  display:flex;align-items:center;gap:10px;white-space:nowrap}
.logo-icon{width:30px;height:30px;border-radius:50%;flex-shrink:0;
...
.sb-link{display:flex;align-items:center;gap:9px;padding:9px 12px;border-radius:10px;font-size:.82rem;font-weight:600;
  color:var(--text2);cursor:pointer;transition:all .14s;border:1px solid transparent;
  background:transparent;font-family:'Outfit',sans-serif;text-align:left;width:100%}
...
.sb-col-actions{padding:6px 10px;display:flex;gap:6px}
...
/* MAIN */
.main{flex:1;min-width:0;display:flex;flex-direction:column;width:0}
...
.auth-box{width:min(480px,100%);background:var(--surface);border:1px solid var(--border2);
  border-radius:22px;padding:clamp(24px,5vw,44px);box-shadow:0 24px 80px rgba(0,0,0,.7);animation:fadeUp .25s ease}
...
/* PAGE */
.page-wrap{display:flex;flex-direction:column;flex:1;min-width:0;width:100%}
...
.page-title{font-family:'Fredoka',sans-serif;font-size:clamp(1.1rem,2.5vw,1.45rem);letter-spacing:2px;text-transform:uppercase;color:var(--text);text-shadow:0 0 20px rgba(200,40,40,.2)}
...
.imp-thumb{border-radius:6px;overflow:hidden;border:1px solid var(--border);aspect-ratio:2.5/3.5;background:var(--surface2)}
...
/* CARD GRID */
.col-wrap{padding:14px 28px 60px;flex:1;min-width:0}
...
.card:hover{border-color:var(--red2);box-shadow:0 12px 36px rgba(200,40,40,.22),0 0 0 1px rgba(200,40,40,.12)}
.card.got{border-color:rgba(58,184,112,.3)}
.card.got:hover{border-color:var(--green);box-shadow:0 12px 36px rgba(58,184,112,.14)}
...
.card-holo{position:absolute;inset:0;border-radius:11px 11px 0 0;pointer-events:none;z-index:3;overflow:hidden;opacity:0;transition:opacity .3s}
.card:hover .card-holo{opacity:1}
.card-holo::after{content:'';position:absolute;inset:-50%;
  background:conic-gradient(from 0deg at var(--mx,50%) var(--my,50%),rgba(255,100,80,.09) 0deg,rgba(255,190,80,.13) 60deg,rgba(80,190,255,.09) 120deg,rgba(190,80,255,.08) 180deg,rgba(255,100,80,.09) 360deg);
  mix-blend-mode:screen}
.card-holo::before{content:'';position:absolute;inset:0;
  background:radial-gradient(circle at var(--mx,50%) var(--my,50%),rgba(255,255,255,.13) 0%,transparent 60%);mix-blend-mode:screen}
.card-badge{position:absolute;top:6px;right:6px;z-index:4;width:20px;height:20px;border-radius:50%;background:var(--green);color:#fff;display:flex;align-items:center;justify-content:center;font-size:.6rem;font-weight:900;box-shadow:0 2px 7px rgba(0,0,0,.5)}
...
/* BROWSE */
.browse-sets-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:12px;padding:0 28px 20px}
.set-card{background:var(--surface);border:1px solid var(--border2);border-radius:var(--radius);padding:16px;cursor:pointer;transition:all .15s}
...
.set-card-badge{display:inline-block;font-size:.63rem;font-weight:700;padding:2px 8px;border-radius:99px;background:rgba(200,40,40,.12);color:var(--red2);border:1px solid rgba(200,40,40,.22);margin-top:8px}
.browse-grid{display:grid;grid-template-columns:repeat(6,1fr);gap:12px;padding:0 28px 60px}
...
.browse-card{width:100%;border-radius:11px;overflow:hidden;background:var(--surface);border:1px solid var(--border2);position:relative;transition:all .18s;animation:fadeUp .2s ease both}
...
.browse-add-btn{position:absolute;inset:0;z-index:3;display:flex;align-items:flex-end;justify-content:center;padding-bottom:10px;background:linear-gradient(to top,rgba(14,12,16,.85) 0%,transparent 50%);opacity:0;transition:opacity .2s;border:none;cursor:pointer}
.browse-card:hover .browse-add-btn{opacity:1}
.browse-add-inner{display:flex;gap:6px;align-items:center;background:linear-gradient(135deg,var(--red),var(--orange));color:#fff;font-family:'Outfit',sans-serif;font-size:.7rem;font-weight:700;padding:5px 12px;border-radius:99px;box-shadow:0 2px 10px rgba(0,0,0,.5)}
...
/* TOAST */
.toast{position:fixed;bottom:24px;right:24px;z-index:999;padding:10px 20px;border-radius:999px;background:var(--surface);border:1px solid;font-size:.8rem;font-weight:600;transform:translateY(50px);opacity:0;transition:all .25s cubic-bezier(.4,0,.2,1);pointer-events:none}
...
function TiltCard({ card, onToggle, onEdit, onDelete, listMode }) {
  ...
  const onMove = useCallback((e) => {
    ...
    el.style.transform = `perspective(700px) rotateX(${(y-.5)*-14}deg) rotateY(${(x-.5)*14}deg) scale3d(1.04,1.04,1.04)`;
    el.style.transition = 'border-color .3s,box-shadow .3s,transform .05s';
    if (holoRef.current) { holoRef.current.style.setProperty('--mx',`${x*100}%`); holoRef.current.style.setProperty('--my',`${y*100}%`); }
  }, [listMode]);
  const onLeave = useCallback(() => {
    if (!ref.current) return;
    ref.current.style.transform = '';
    ref.current.style.transition = 'border-color .3s,box-shadow .3s,transform .5s cubic-bezier(.23,1,.32,1)';
  }, []);
  return (
    <div ref={ref} className={`card${card.obtained?' got':''}`} onMouseMove={onMove} onMouseLeave={onLeave}>
      <div className="card-img-box">
        {card.src ? <img className="card-img" src={card.src} alt={card.name} loading="lazy"/> : <div style={{width:'100%',height:'100%',background:'var(--surface2)',display:'flex',alignItems:'center',justifyContent:'center',opacity:.15,fontSize:'2rem'}}>🃏</div>}
      ...
      <div className="card-foot">
        ...
        <div className="card-acts">
          <button className={`btn btn-sm${card.obtained?' btn-ok':''}`} onClick={()=>onToggle(card.id)}>{card.obtained?'✓ Obtenue':'+ Marquer'}</button>
...
function CollectionPage({ collection, allCollections, onUpdate, onAddToCollection, showToast }) {
  ...
  const [layout,setLayout]=useState('g6'); const [filter,setFilter]=useState('all');
  const [modal,setModal]=useState(null);
  const [impOpen,setImpOpen]=useState(false); const [impSets,setImpSets]=useState([]); const [impSetId,setImpSetId]=useState('');
  const [impState,setImpState]=useState('idle'); const [impCards,setImpCards]=useState([]); const [impErr,setImpErr]=useState('');
  const [drag,setDrag]=useState(false); const [pickModal,setPickModal]=useState(null);
  ...
  const fetchPreview=async()=>{
    ...
    try{ const res=await fetch(`${TCGDEX}/sets/${impSetId}`); if(!res.ok) throw new Error(`Erreur ${res.status}`); const data=await res.json(); if(!data.cards?.length) throw new Error('Aucune carte trouvée.'); setImpCards(data.cards); setImpState('preview'); }
    ...
    <div className="page-wrap">
      <div className="page-hdr">
        ...
        <div className="page-hdr-actions">
          ...
          <button className="btn" onClick={()=>fileRef.current?.click()}>＋ Ajouter des photos</button>
      ...
      <div className="imp-panel">
        ...
        {impOpen&&<div className="imp-body">
          {impSets.length===0?<div className="spinner-wrap"><div className="spinner"/>Chargement des extensions…</div>:<>
            ...
            <p className="imp-info">Images fournies par <strong>api.tcgdex.net</strong> — open source, en français. Les doublons sont ignorés.</p>
          </>}
          ...
          {impState==='preview'&&<div>
            ...
            <div className="imp-preview">{impCards.map(c=><div key={c.id} className="imp-thumb">{c.image?<img src={`${c.image}/high.webp`} alt={c.name} loading="lazy"/>:<div style={{background:'var(--surface2)',width:'100%',height:'100%'}}/>}</div>)}</div>
      ...
      <div className="col-wrap">
        ...
        {visible.length===0
          ...
          :<div className={`grid ${layout}`}>{visible.map((card,i)=><div key={card.id} style={{animationDelay:`${Math.min(i*.025,.5)}s`}}><TiltCard card={card} onToggle={toggleObtained} onEdit={openEdit} onDelete={deleteCard} listMode={layout==='glist'}/></div>)}</div>
        }
      </div>
...
function BrowsePage({ allCollections, onAddToCollection, showToast }) {
  const [sets,setSets]=useState([]); const [selSet,setSelSet]=useState(null); const [setCards,setSetCards]=useState([]);
  const [loading,setLoading]=useState(true); const [loadingC,setLoadingC]=useState(false); const [pickModal,setPickModal]=useState(null);

  useEffect(()=>{ fetch(`${TCGDEX}/sets`).then(r=>r.json()).then(d=>{setSets(d);setLoading(false);}).catch(()=>setLoading(false)); },[]);

  const loadSet=async s=>{ setSelSet(s);setLoadingC(true);setSetCards([]); try{ const r=await fetch(`${TCGDEX}/sets/${s.id}`); const d=await r.json(); setSetCards(d.cards||[]); }catch{} setLoadingC(false); };
  const inAny=tcgId=>allCollections.some(col=>col.cards.some(c=>c.tcgId===tcgId));
  const handleAdd=(c,setName)=>{ if(inAny(c.id)){showToast('Déjà dans une collection','#c8a448');return;} if(allCollections.length>1) setPickModal({card:c,setName}); else if(allCollections.length===1) doAdd(allCollections[0].id,c,setName); };
  const doAdd=(colId,c,setName)=>{ onAddToCollection(colId,[{id:`tcg-${c.id}-${Math.random()}`,tcgId:c.id,src:c.image?`${c.image}/high.webp`:'',name:c.name||c.id,series:setName,number:c.localId||'',obtained:false}]); showToast(`${c.name} ajouté ✦`); setPickModal(null); };
  ...
  return (
    <div className="page-wrap">
      <div className="page-hdr">
        <div><div className="page-title">Parcourir les <span>Extensions</span></div>
      ...
      {!selSet
        ? loading?<div className="spinner-wrap" style={{padding:40}}><div className="spinner"/>Chargement…</div>
          :<div className="browse-sets-grid">{displaySets.map(s=><div key={s.id} className="set-card" onClick={()=>loadSet(s)}><div className="set-card-name">{s.name}</div><div className="set-card-meta">{s.cardCount?.total??'?'} cartes · {s.releaseDate?.split('-')[0]??'—'}</div><div className="set-card-badge">{s.id}</div></div>)}</div>
        : loadingC?<div className="spinner-wrap" style={{padding:40}}><div className="spinner"/>Chargement des cartes…</div>
          :<div className="browse-grid">{setCards.map((c,i)=><div key={c.id} className="browse-card" style={{animationDelay:`${Math.min(i*.02,.4)}s`}}>
              <div className="card-img-box">{c.image?<img className="card-img" src={`${c.image}/high.webp`} alt={c.name} loading="lazy" style={{filter:'none'}}/>:<div style={{width:'100%',height:'100%',background:'var(--surface2)'}}/>}
                {inAny(c.id)?<div className="browse-in-col">✓</div>:<button className="browse-add-btn" onClick={()=>handleAdd(c,selSet.name)}><div className="browse-add-inner">＋ Ajouter</div></button>}
              </div>
              <div className="card-foot"><div className="card-name">{c.name||c.id}</div><div className="card-meta">{c.localId}</div></div>
            </div>)}
          </div>
      }
      <PickCollectionModal open={!!pickModal} collections={allCollections} defaultId={allCollections[0]?.id} onClose={()=>setPickModal(null)} onConfirm={colId=>doAdd(colId,pickModal.card,pickModal.setName)}/>
    </div>
  );
}
...
function AccountPage({ user, profile, onProfileUpdate, showToast, onLogout }) {
  const [pseudo,setPseudo]=useState(profile?.pseudo||''); const [oldPw,setOldPw]=useState(''); const [newPw,setNewPw]=useState('');
  const [l1,setL1]=useState(false); const [l2,setL2]=useState(false); const [err1,setErr1]=useState(''); const [err2,setErr2]=useState('');
  const savePseudo=async()=>{ if(pseudo.length<3) return setErr1('Au moins 3 caractères.'); setL1(true);setErr1(''); const{error}=await supabase.from('profiles').update({pseudo}).eq('id',user.id); setL1(false); if(error) return setErr1(error.message); onProfileUpdate({...profile,pseudo}); showToast('Pseudo mis à jour ✦'); };
  const savePassword=async()=>{ if(newPw.length<8) return setErr2('Au moins 8 caractères.'); setL2(true);setErr2(''); const{error:e1}=await supabase.auth.signInWithPassword({email:user.email,password:oldPw}); if(e1){setL2(false);return setErr2('Mot de passe actuel incorrect.');} const{error}=await supabase.auth.updateUser({password:newPw}); setL2(false); if(error) return setErr2(error.message); setOldPw('');setNewPw(''); showToast('Mot de passe mis à jour ✦'); };
  return (
    <div className="page-wrap">
...
function AppShell({ user, profile, onProfileUpdate, onLogout, showToast }) {
  ...
  const persist=useCallback((col)=>{ clearTimeout(saveTimers.current[col.id]); saveTimers.current[col.id]=setTimeout(async()=>{ await supabase.from('collections').update({cards:col.cards,updated_at:new Date().toISOString()}).eq('id',col.id); },1200); },[]);
  const updateCollection=useCallback((updated)=>{ setCollections(prev=>prev.map(c=>c.id===updated.id?updated:c)); persist(updated); },[persist]);
  const addToCollection=useCallback((colId,newCards)=>{ setCollections(prev=>prev.map(c=>{ if(c.id!==colId) return c; const u={...c,cards:[...c.cards,...newCards]}; persist(u); return u; })); },[persist]);

  const createCollection=async()=>{ const name=(newColName.trim()||'Nouvelle collection'); const{data,error}=await supabase.from('collections').insert({user_id:user.id,name,cards:[]}).select().single(); if(!error&&data){setCollections(prev=>[...prev,data]);setActiveColId(data.id);setPage('collection');} setNewColModal(false);setNewColName(''); };
  const renameCollection=async()=>{ if(!renameModal?.name?.trim()) return; await supabase.from('collections').update({name:renameModal.name}).eq('id',renameModal.id); setCollections(prev=>prev.map(c=>c.id===renameModal.id?{...c,name:renameModal.name}:c)); setRenameModal(null); showToast('Collection renommée ✦'); };
  const deleteCollection=async(colId)=>{ if(!confirm('Supprimer cette collection ?')) return; await supabase.from('collections').delete().eq('id',colId); const next=collections.filter(c=>c.id!==colId); setCollections(next); if(activeColId===colId) setActiveColId(next[0]?.id||null); showToast('Collection supprimée','#c82828'); };
  ...
  const sidebar=(
    <div className="sidebar">
      ...
      <nav className="sb-nav">
        ...
        {collections.map(col=>(
          <button key={col.id} className={`sb-link${page==='collection'&&activeColId===col.id?' active':''}`} onClick={()=>{setActiveColId(col.id);setPage('collection');setMob(false);}}>
  ...
  return (
    <div className="app-shell">
      ...
      <div className={`overlay${newColModal?' open':''}`} onClick={e=>e.target.classList.contains('overlay')&&setNewColModal(false)}>
        <div className="modal"><div className="modal-title">＋ Nouvelle collection</div>
          <div className="field"><label>Nom de la collection</label><input autoFocus value={newColName} onChange={e=>setNewColName(e.target.value)} placeholder="Ex : Espèces Delta" onKeyDown={e=>e.key==='Enter'&&createCollection()}/></div>
      ...
      <div className={`overlay${renameModal?' open':''}`} onClick={e=>e.target.classList.contains('overlay')&&setRenameModal(null)}>
        {renameModal&&<div className="modal"><div className="modal-title">✏️ Renommer</div>
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    supabase.auth.getSession().then(async({data})=>{ if(data.session?.user){ const u=data.session.user; setUser(u); const{data:p}=await supabase.from('profiles').select('*').eq('id',u.id).single(); setProfile(p); } setLoading(false); });
    const{data:sub}=supabase.auth.onAuthStateChange(async(event,session)=>{
      if(event==='SIGNED_IN'&&session?.user){ setUser(session.user); const{data:p}=await supabase.from('profiles').select('*').eq('id',session.user.id).single(); setProfile(p); setIsReset(false); window.history.replaceState({},'',window.location.pathname); }
  ...
  return (
    <>
      <style>{CSS}</style>
      {loading
        ?<div style={{position:'fixed',inset:0,display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column',gap:16,background:'var(--bg)'}}><div className="logo"><div className="logo-icon">◆</div>Cartodex</div><div className="spinner"/></div>
